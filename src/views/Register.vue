<template>
  <div class="auth-card">
    ...
    <form @submit.prevent="onSubmit" novalidate>
      ...
      <input v-model="password" type="password" placeholder="Password (min 6 chars)" autocomplete="new-password" required />
      <input v-model="confirm" type="password" placeholder="Confirm Password" autocomplete="new-password" required />
...
async function onSubmit () {
  ...
  if (!email.value || !password.value || !confirm.value) { error.value='Please complete all fields.'; return }
  if (!emailRe.test(email.value)) { error.value='Please enter a valid email.'; return }
...
.ok{margin-top:10px;color:#2e7d32}
</style>
