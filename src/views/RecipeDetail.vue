...
<template>
  <div class="wrap" v-if="item">
    ...
    <p style="margin:8px 0 16px;">
      Total: <strong>{{ item.snapshot.total.kcal }}</strong> kcal ·
      P {{ item.snapshot.total.protein }} · F {{ item.snapshot.total.fat }} · C {{ item.snapshot.total.carbs }}
    </p>

    <div class="grid">
      <div v-for="(val,day) in item.snapshot.days" :key="day" class="card">
        <h3>{{ day }}</h3>
        <p>Day total: {{ val.total.kcal }} kcal ·
           P {{ val.total.protein }} · F {{ val.total.fat }} · C {{ val.total.carbs }}</p>
        <ul>
